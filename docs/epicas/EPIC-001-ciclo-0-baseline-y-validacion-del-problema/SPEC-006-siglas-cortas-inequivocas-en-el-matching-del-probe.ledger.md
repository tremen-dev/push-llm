...
| CA-7 | `probe/README.md` (sección *Brand matching (RN-01, RN-11)* y descripción de `summary.md`), docstring de `probe/matching.py` | `python -m pytest probe/tests` → 94 passed (2026-09-24, sin claves ni red: 0 variables `*API_KEY*` en el entorno); `ruff check probe` → All checks passed | `python -m pytest probe/tests` → 94 passed (0 variables *API_KEY* en entorno); `ruff check probe` → All checks passed; README y docstring describen `exact_aliases` con referencia a RN-11/ADR-002 | ✅ |
